...
**Batch mode limitations:** Browser backends (browser_use/Playwright) are not available, so posting freshness signals (exact days posted, apply button state) cannot be directly verified. Mark these as "unverified (batch mode)."
