...
| browser_use | 推奨ブラウザバックエンド（headless Playwright）。求人がまだアクティブか検証、レンダリングされた JD 抽出、フォーム読み取り。**クリティカル：2 つ以上のブラウザエージェントを並列起動しない。** |
| Playwright | browser_use が利用できない場合のフォールバック。 |
