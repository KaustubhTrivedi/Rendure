...
2. **Para cada URL pendiente**:
   ...
   b. **Extraer JD** usando browser_use → Playwright → WebFetch → WebSearch
...
1. **browser_use (preferido):** Navegar y leer el JD renderizado. Funciona con todas las SPAs.
2. **Playwright (fallback):** Si browser_use no está disponible, usar Playwright.
3. **WebFetch (fallback):** Para páginas estáticas o cuando no hay acceso a browser.
