...
2. **Для каждого URL**:
   ...
   b. **Извлечь JD** через browser_use → Playwright → WebFetch → WebSearch
...
3. **Если 3+ URL**, запустить агентов параллельно (Agent tool с `run_in_background`). **Ограничение:** browser_use/Playwright требует ресурсов — использовать **только один** browser-агент одновременно (правило `_shared.md`). Все прочие шаги (WebFetch, оценка, генерация отчёта) допускают полную параллелизацию. Рекомендуемая схема: один агент с browser_use/Playwright верифицирует активность вакансии; остальные агенты получают JD через WebFetch и параллельно проводят оценку.
...
1. **browser_use (предпочтительно):** Навигация и чтение рендеринга JD. Работает со всеми SPA.
2. **Playwright (fallback):** Если browser_use недоступен.
